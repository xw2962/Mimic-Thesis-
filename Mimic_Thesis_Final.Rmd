...
icu_wound_data <- diabetic_icu_stays %>%
  # Join wound-related lab results using subject_id and stay_id
  left_join(wound_healing_labs, by = c("subject_id", "stay_id")) %>%
  
  # Join wound-related procedures using subject_id and stay_id.
  # Note: This join creates duplicate 'hadm_id' columns.
  left_join(wound_procedures, by = c("subject_id", "stay_id")) %>%
  
  # Restore the original hadm_id by renaming and dropping the duplicate.
  # Here, we assume that 'hadm_id.x' (from diabetic_icu_stays) is the desired column.
  rename(hadm_id = hadm_id.x) %>%
  select(-hadm_id.y) %>%
  
  # Join wound-related microbiology results using subject_id and hadm_id.
  left_join(wound_results, by = c("subject_id", "hadm_id")) %>%
  
  # Create a flag for prolonged ICU stay (>7 days)
  mutate(prolonged_stay = if_else(los > 7, 1, 0))

# Check for any empty column names and replace them with a placeholder ("unknown")
...
# Check for the existence of 'value.x' and 'value.y'. If they exist, convert and coalesce them.
if (all(c("value.x", "value.y") %in% names(icu_wound_data))) {
  ...
  # If there is already a column named "value", convert it to numeric
  icu_wound_data <- icu_wound_data %>%
    mutate(value = as.numeric(if_else(value %in% c("N/A", ""),
                                      NA_character_,
                                      as.character(value))))
} else {
  # If none of the expected value columns exist, create a placeholder column
...
# Remove rows where 'value' or 'los' is missing.
icu_wound_data <- icu_wound_data %>%
  filter(!is.na(value) & !is.na(los))

# Optionally, impute any remaining missing 'value' with the mean (this might be redundant after filtering)
icu_wound_data <- icu_wound_data %>%
  mutate(value = if_else(is.na(value), mean(value, na.rm = TRUE), value))

# (D) Select only the necessary columns for further analysis
icu_wound_data <- icu_wound_data %>%
  select(subject_id, stay_id, los, value, test_name, org_name, prolonged_stay)

# Verify the data structure and summary statistics
str(icu_wound_data)
